...
- [Development guidelines](#development-guidelines)
    - [General 2chcoin Core](#general-2chcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
2chcoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/2chcoinpay/2chcoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_2chcoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_2chcoin --log_level=test_suite
$ valgrind -v --leak-check=full src/2chcoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_2chcoin.coverage/index.html`.
```

**Sanitizers**

2chcoin Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as 2chcoin Core, where everyone uses
...
pay attention to for reviewers of 2chcoin Core code.

General 2chcoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: 2chcoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against 2chcoin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `2chcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `2chcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of 2chcoin Core, the wallet was always
