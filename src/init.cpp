...
public:
    ...
    // 2chcoin Specific WalletInitInterface InitCoinJoinSettings
 ...
 * between 2chcoind, and 2chcoin-qt and non-server tools.
*/
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-sporkaddr=<2chcoinaddress>", "Override spork address. Only useful for regtest and devnet. Using this on mainnet or testnet will ban you.", false, OptionsCategory::DEBUG_TEST);
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by 2chcoin Platform, to the specified username.", false, OptionsCategory::MASTERNODE);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/2chcoinpay/2chcoin>";
    const std::string URL_WEBSITE = "<https://2chcoin.org>";
...
/** Sanity checks
 *  Ensure that 2chcoin Core is running in a usable environment with all
...
{
    // Make sure only a single 2chcoin Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if 2chcoin Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if 2chcoin Core is started while in a temporary directory.\n",
        ...
        do {
            ...
            try {
                ...
                if (!fDisableGovernance && !fTxIndex
                   && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/2chcoinpay/2chcoin/pull/1817 and https://github.com/2chcoinpay/2chcoin/pull/1743
    ...
    // ********************************************************* Step 10c: schedule 2chcoin-specific tasks
