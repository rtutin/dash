...
{
    return QCoreApplication::translate("2chcoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in 2chcoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in 2chcoin.qrc)
...
/** Class encapsulating 2chcoin Core startup and shutdown.
...
/** Main 2chcoin application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // 2chcoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. 2chcoin Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"2chcoin Core - \""), false, OptionsCategory::GUI);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(2chcoin);
    Q_INIT_RESOURCE(2chcoin_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse 2chcoin.conf
    ...
    // 2chcoin: links repeatedly have their payment requests routed to this process:
