...
void OptionsModel::Init(bool resetSettings)
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::_2chcoin);
    ...
    if (!settings.contains("nCoinJoinAmount")) {
        ...
        if (!settings.contains("nAnonymize2chcoinAmount"))
            settings.setValue("nCoinJoinAmount", DEFAULT_COINJOIN_AMOUNT);
        else
            settings.setValue("nCoinJoinAmount", settings.value("nAnonymize2chcoinAmount").toInt());
    }
