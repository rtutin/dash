...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 2chcoin irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on 2chcoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in 2chcoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("2chcoin.conf", statusObj);
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by masternode configured in 2chcoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* 2chcoin features */
    { "2chcoin",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "2chcoin",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "2chcoin",               "gobject",                &gobject,                {} },
    { "2chcoin",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
