...
def 2chcoinhash(s):
    return 2chcoin_hash.getPoWHash(s)
...
# Objects that map to 2chcoind objects, which can be serialized/deserialized
...
class CBlockHeader():
    ...
    def calc_sha256(self):
        if self.sha256 is None:
            ...
            r += struct.pack("<I", self.nBits)
            r += struct.pack("<I", self.nNonce)
            self.sha256 = uint256_from_str(2chcoinhash(r))
            self.hash = encode(2chcoinhash(r)[::-1], 'hex_codec').decode('ascii')
...
class msg_headers():
    ...
    def deserialize(self, f):
        # comment in 2chcoind indicates these should be deserialized as blocks
